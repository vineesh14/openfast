...
SUBROUTINE BD_Init( InitInp, u, p, x, xd, z, OtherState, y, MiscVar, Interval, InitOut, ErrStat, ErrMsg )
  ...
  ! Initialize ErrStat
   ...
   IF(p%quadrature .EQ. GAUSS_QUADRATURE) THEN
       ...
       CALL BD_GaussPointWeight(p%nqp,p%QPtN,p%QPtWeight,p%QPtWghtDeltaEta,ErrStat2,ErrMsg2) !calculates p%QPtN, p%QPtWeight, p%QPtWghtDeltaEta
          CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
          if (ErrStat >= AbortErrLev) then
             call cleanup()
             return
   ...
   ELSEIF(p%quadrature .EQ. TRAP_QUADRATURE) THEN
      ...
      CALL BD_TrapezoidalPointWeight(p, InputFileData)        ! computes p%QPtN, p%QPtWeight, and p%QPtWghtDeltaEta
...
subroutine InitializeMassStiffnessMatrices(InputFileData,p,ErrStat, ErrMsg)
   ...
   INTEGER(IntKi)                               :: idx_FE            !< index of current FE point in loop
...
!FIXME: if we must shift the K and M matrices, this will need to be recaulculated (or calculated elsewhere)
   ...
   ENDDO
      ...
      !---------------------------------------------------------------------------------------------------------------------------------------
      ! To speed up the output of the internal forces and moments, we are going to calculate the interpolated mass value at each GLL.  This is
      ! for finding the center of mass location for each segment when integrating with FE output nodes.
      ! InterpMassAtFE -- Calculate the interpolated mass (from mass0(1,1,:)) at the GLL locations.  This is for integration of output loads
   if (p%BldMotionNodeLoc == BD_MESH_FE) then
      !.... Compute the relative location of each quadrature node along the length of the curved blade ....
       CALL AllocAry(temp_ratio,p%nodes_per_elem,p%elem_total,'temp_ratio',ErrStat2,ErrMsg2)
          CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
          if (ErrStat >= AbortErrLev) then
             call cleanup()
             return
      ...
      DO idx_FE=1,p%nodes_per_elem
         temp_ratio(idx_FE,1) = ((p%GLL_Nodes(idx_FE) + 1.0_BDKi)/2.0_BDKi)*p%member_eta(1)  ! get GLL_Nodes ratio in [0,1] and multiply by member (element)'s relative length along the beam [0,1]
      ENDDO
      DO i=2,p%elem_total
         ! add lengths of all previous members (elements)
         DO j=1,i-1
         ...
         ! then add ratio of length of quadrature point along this member (element)
         DO idx_FE=1,p%nodes_per_elem
               temp_ratio(idx_FE,i) = temp_ratio(idx_FE,i) + ((p%GLL_Nodes(idx_FE) + 1.0_BDKi)/2.0_BDKi)*p%member_eta(i)
         ENDDO
      ENDDO

      !.... now compute interpolated mass density for GLL point
      DO i=1,p%elem_total
         DO idx_FE=1,p%nodes_per_elem
            DO k=1,InputFileData%InpBl%station_total

               IF(temp_ratio(idx_FE,i) - InputFileData%InpBl%station_eta(k) <= EPS) THEN
                     ! If a mass matrix was specified at this GLL location
                  IF(ABS(temp_ratio(idx_FE,i) - InputFileData%InpBl%station_eta(k)) <= EPS) THEN
                           p%InterpMassAtFE(idx_FE,i) = InputFileData%InpBl%mass0(1,1,k)
                  ELSE  ! otherwise calculate this value.
                           temp = (InputFileData%InpBl%mass0(1,1,k)-InputFileData%InpBl%mass0(1,1,k-1)) / &
                                  (InputFileData%InpBl%station_eta(k) - InputFileData%InpBl%station_eta(k-1))
                           p%InterpMassAtFE(idx_FE,i) = temp * temp_ratio(idx_FE,i) + &
                                                         InputFileData%InpBl%mass0(1,1,k-1) - &
                                                         temp * InputFileData%InpBl%station_eta(k-1)
                  ENDIF
                  EXIT
               ENDIF
            ENDDO ! k=InputFileData%InpBl%station_total
       ...
       DEALLOCATE(temp_ratio)
   else  ! Mesh is on QP, so we don't need the p%InterpMassAtFE values
       p%InterpMassAtFE = 0.0_BDKi
   endif
...
subroutine InitializeNodalLocations(InputFileData,p,ErrStat, ErrMsg)
   ...
   ! NOTE: InputFileData%member_total = p%elem_total
   CALL BD_SegmentEta(InputFileData%member_total,InputFileData%kp_member,InputFileData%kp_coordinate,p%SP_Coef,p%segment_eta,p%zToEtaMapping,ErrStat2,ErrMsg2)
      CALL SetErrStat(ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName)
      if (ErrStat >= AbortErrLev) return
...
subroutine SetParameters(InitInp, InputFileData, p, ErrStat, ErrMsg)
   ...
   CALL AllocAry(p%uup0, p%dof_node,    p%nqp,             p%elem_total,'p%uup0',ErrStat2,ErrMsg2); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
   ...
   CALL AllocAry( p%InterpMassAtFE,              p%nodes_per_elem, p%elem_total, 'p%InterpMassAtFE -- optimzation',     ErrStat2, ErrMsg2 ); CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
   ...
   CALL AllocAry(p%QPtWghtDeltaEta,p%nqp,'p%QPtWghtDeltaEta array',ErrStat2,ErrMsg2) ; CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, RoutineName )
...
SUBROUTINE BD_CalcOutput( t, u, p, x, xd, z, OtherState, y, m, ErrStat, ErrMsg )
   ...
   IF(p%analysis_type /= BD_STATIC_ANALYSIS) THEN ! dynamic analysis
      ...
      CALL BD_QPDataAcceleration( p, m%RHS, m )     ! Naaa --> aaa (OtherState%Acc --> m%qp%aaa)
      DO nelem=1,p%elem_total
         CALL BD_InertialForce( nelem,p,m,.false. )    ! Calculate Fi
      ENDDO
   ELSE
   ...
   DO nelem = 1,p%elem_total
      DO idx_qp = 1,p%nqp
         ...
         DO idx_node=1,p%nodes_per_elem
            ...
            rrp(1:3)                 = rrp(1:3)                 + p%ShpDer(idx_node,idx_qp)/p%Jacobian(idx_qp,nelem)*p%rrN0(1:3,idx_node,nelem)
         ENDDO


            !> Calculate the initial tangent tensor, \f$\underline{\underline{H}}(\underline{r})\f$ times \f$\underline{r}^\prime\f$
            !! This term gives us the axial(R'R^T).  See Bauchau book 17.119
         CALL BD_CrvMatrixH(p%uu0(4:6,idx_qp,nelem),temp33)         ! retrieve the tangent tensor given rrr
         p%uup0(4:6,idx_qp,nelem) = MATMUL(temp33,rrp)              ! \underline{k} in eq (5) http://www.nrel.gov/docs/fy14osti/60759.pdf


            !> Add the blade root rotation parameters. That is, compose the rotation
            !! parameters calculated with the shape functions with the rotation parameters
            !! for the blade root.
         ...
         CALL BD_CrvCompose(rot_temp,rotu_temp,rot0_temp,FLAG_R1R2)  ! rot_temp = rot0_temp composed with rotu_temp
...
SUBROUTINE BD_DisplacementQP( nelem, p, x, m )
   ...
   DO idx_qp=1,p%nqp
         ...
         DO idx_node=1,p%nodes_per_elem
            ...
            m%qp%uup(1:3,idx_qp,nelem) = m%qp%uup(1:3,idx_qp,nelem)  + p%ShpDer(idx_node,idx_qp)/p%Jacobian(idx_qp,nelem)*(x%q(1:3,elem_start - 1 + idx_node)-x%q(1:3,elem_start))
         ENDDO
   ...
   if (.not. fact) then
      ...
      do idx_qp=1,p%nqp
         ...
         call Calc_Fc_Fd()
            ...
            !!       \end{bmatrix}
            !! \f$
         Wrk33(:,:) = OuterProduct(m%qp%RR0(1:3,3,idx_qp,nelem), m%qp%RR0(1:3,3,idx_qp,nelem))     ! material z-direction
...
contains
   subroutine Calc_Fc_Fd()
     ...
     !REAL(BDKi)                                   :: Wrk(3)
         ...
         !!
         !! Note: \f$ \underline{\underline{R}}\underline{\underline{R}}_0 \f$ is used to go from the material basis into the inertial basis
         !!       and the transpose for the other direction. Here we are finding what the rigid body rotation of the original curvature induces
         !!       and subtracting that.  Bauchau is rather nebulous in his description of this process.
      eee(1:3) = m%qp%E1(1:3,idx_qp,nelem) - m%qp%RR0(1:3,3,idx_qp,nelem)
      ...
      !epsilon_{1} in material basis (for major axis of blade, which is z in the IEC formulation)
      e1s = dot_product( m%qp%RR0(:,3,idx_qp,nelem), eee(1:3) )

      ! kappa_{1} in material basis (for major axis of blade, which is z in the IEC formulation)
      k1s = dot_product( m%qp%RR0(1:3,3,idx_qp,nelem), eee(4:6) )
...
SUBROUTINE BD_InertialForce( nelem, p, m, fact )
   ...
   do idx_qp=1,p%nqp
      ...
      m%qp%Fi(1:3,idx_qp,nelem) = p%qp%mmm(idx_qp,nelem)*m%qp%aaa(1:3,idx_qp,nelem) &
                                ...
                                + cross_product(m%qp%vvv(4:6,idx_qp,nelem), beta)
      m%qp%Fi(4:6,idx_qp,nelem) = cross_product(m%qp%RR0mEta(:,idx_qp,nelem), m%qp%aaa(1:3,idx_qp,nelem)) + nu + cross_product(m%qp%vvv(4:6,idx_qp,nelem), gama)
   end do
...
!  This is from Bauchau's book, eq 17.112c
...
SUBROUTINE BD_SegmentEta(member_total, kp_member, kp_coordinate, SP_Coef, segment_eta, zToEtaMapping, ErrStat, ErrMsg)
   ...
   INTEGER(IntKi), parameter   :: sample_total = 100

   INTEGER(IntKi)              :: i
   INTEGER(IntKi)              :: j
   INTEGER(IntKi)              :: k
   INTEGER(IntKi)              :: m
   INTEGER(IntKi)              :: mappingIdx
   INTEGER(IntKi)              :: temp_id
   INTEGER(IntKi)              :: id0
   INTEGER(IntKi)              :: id1
...
SUBROUTINE BD_StaticSolution( x, gravity, p, m, piter, ErrStat, ErrMsg )
   ...
   DO piter=1,p%niter
      ...
      ELSE
            ! Due to roundoff errors and precision, the first criteria is
            ! sometims not achieved even though we have a valid solution
...
SUBROUTINE BD_InternalForceMomentIGE( x, p, m )
   ...
   REAL(BDKi)                    :: Tmp6(6)
   REAL(BDKi)                    :: ContribThisQPdistr(6)      ! This QP's distributed load contribution
   REAL(BDKi)                    :: ContribNextQPdistr(6)      ! The next outboard QP's distributed load contribution
   REAL(BDKi)                    :: ContribThisQPmass(6)       ! This QP's mass forces contribution
   REAL(BDKi)                    :: ContribNextQPmass(6)       ! The next outboard QP's mass forces contribution
   REAL(BDKi)                    :: ForceQPrangeDistr(6)       ! Force from distributed loads for this range of QP
   REAL(BDKi)                    :: ForceQPrangeMass(6)        ! Force from mass terms for this range of QP
   REAL(BDKi)                    :: MomentArmPoint(3)          ! Moment arm for point load
   REAL(BDKi)                    :: MomentArmDistr(3)          ! Moment arm for distributed force contribution
   REAL(BDKi)                    :: MomentArmMass(3)           ! Moment arm for center of mass location
   REAL(BDKi)                    :: NodeMass                   ! Mass density times weighting associated with this node
   REAL(BDKi)                    :: MassEnd                    ! Mass density associated with end point of integration range on FE summation
   REAL(BDKi)                    :: MassThisFE                 ! Mass density associated with GLL for weighting in integration of forces on FE summation
   REAL(BDKi)                    :: PrevNodeMass               ! Mass density times weighting associated with previous node
...
! NOTE: the following will need attention when we add multi-element trapezoidal quadrature capability.
      ...
      DO idx_node=size(p%NdIndx),1,-1
         ...
         do idx_FE=p%nodes_per_elem,2,-1     ! Skip first FE point since it overlaps with the last FE of the previous element
            if ( p%FEoutboardOfQPt(idx_FE,idx_qp) ) then    ! This if statement makes loop optimization impossible -- but we don't have many FE nodes
               ...
               MomentArmPoint =  (p%uuN0(1:3,idx_FE,nelem) + x%q(1:3,idx_FE_all)) - (p%uu0(1:3,idx_qp,nelem) + m%qp%uuu(1:3,idx_qp,nelem))
               ...
               m%BldInternalForceQP(4:6,idx_node)  =  m%BldInternalForceQP(4:6,idx_node) + Tmp6(4:6)  &
                                                   +  cross_product( MomentArmPoint, Tmp6(1:3))
            endif
         enddo
      ...
      PrevNodeMass   =  p%qp%mmm(p%nqp,p%elem_total) * p%QPtWeight(p%nqp) * p%Jacobian(p%nqp,p%elem_total)
...
!FIXME:TrapMultiElem when converting to multiple elements, make sure this still works
      ...
      DO idx_node=size(p%NdIndx)-1,1,-1
         ...
         ContribThisQPdistr   =  p%Jacobian(idx_qp,nelem) * m%DistrLoad_QP(1:6,idx_qp,nelem)
         ContribThisQPmass    =  p%Jacobian(idx_qp,nelem) * ( m%qp%Fg(1:6,idx_qp,nelem) - m%qp%Fi(1:6,idx_qp,nelem) )

            ! Force and moment from this QP range
            ! Add the contributions from this node and the next node and apply trap weighting based on span (stored in QPtWghtDeltaEta).
            ! NOTE: the pointloads have already been applied at this QP.
         ForceQPrangeDistr = p%QPtWghtDeltaEta(idx_qp) * ( ContribThisQPdistr + ContribNextQPdistr )
         ForceQPrangeMass  = p%QPtWghtDeltaEta(idx_qp) * ( ContribThisQPmass  + ContribNextQPmass  )
         m%BldInternalForceQP(:,idx_node) =  m%BldInternalForceQP(:,idx_node) &
                                          +  ForceQPrangeDistr                &
                                          +  ForceQPrangeMass
         ...
         NodePos = p%uu0(1:3,idx_qp,nelem) + m%qp%uuu(1:3,idx_qp,nelem)
         MomentArmPoint = PrevNodePos - NodePos
         ...
         m%BldInternalForceQP(4:6,idx_node)  =  m%BldInternalForceQP(4:6,idx_node)                                   &
                                             +  m%BldInternalForceQP(4:6,idx_node+1)                                 &  ! Next node outboard moments
                                             +  cross_product( MomentArmDistr, ForceQPrangeDistr(1:3) )              &  ! Distributed load contributions between this and next node
                                             +  cross_product( MomentArmMass , ForceQPrangeMass(1:3)  )              &  ! Mass effect contributions between this and next node
                                             +  cross_product( MomentArmPoint, m%BldInternalForceQP(1:3,idx_node+1) )   ! Force induced moment from next node outboard

            ! Keep track of node position next node in.
         PrevNodePos    =  NodePos
         PrevNodeMass   =  NodeMass
   ...
   CASE (BD_MESH_FE)         ! For an output mesh at the FE points
      ...
      !        The BldInternalForceFE has p%node_total {= p%elem_total*(p%nodes_per_elem - 1) + 1} nodes.  This matches all the
      !        FE node arrays (x%q, etc).
      ...
      m%BldInternalForceFE = 0.0_BDKi
         ...
         ! Tip FE node must be handled separately
      idx_FE_all = p%node_total     ! Last node on current element
      PrevNodePos = p%uuN0(1:3,p%nodes_per_elem,p%elem_total) + x%q(1:3,idx_FE_all)

         ! Add any discrete tip pointloads (force and moment)
      m%BldInternalForceFE(1:6,p%node_total) = m%PointLoadLcl(1:6,idx_FE_all)
      ...
      DO nelem = p%elem_total,1,-1
         ...
         if ( p%QPtN(p%nqp)      < p%GLL_Nodes(p%nodes_per_elem) ) then
            ...
            MassThisFE=p%InterpMassAtFE(idx_FE,nelem)

               ! Forces at this QP
            ContribThisQPdistr = p%Jacobian(idx_qp,nelem) * ( m%DistrLoad_QP(1:6,idx_qp,nelem) )
            ContribThisQPmass  = p%Jacobian(idx_qp,nelem) * ( m%qp%Fg(1:6,idx_qp,nelem) - m%qp%Fi(1:6,idx_qp,nelem) )
            ForceQPrangeDistr = p%QPtWghtIntForceFE(idx_qp  ,2,idx_FE,nelem) * ContribThisQPdistr   ! Force from distributed load outboard of this QP
            ForceQPrangeMass  = p%QPtWghtIntForceFE(idx_qp  ,2,idx_FE,nelem) * ContribThisQPmass    ! Force from mass effects outboard of this QP

               ! moment arm for force to moment contribution (midpoint of the integration range for this piece to the previous FE)
            MomentArmDistr =  ( (p%uuN0(1:3,idx_FE  ,nelem) + x%q(1:3,idx_FE_all  ))                     &
            ...
            m%BldInternalForceFE(1:6,idx_FE_all-1  )  =  m%BldInternalForceFE(1:6,idx_FE_all-1  )                 &
                                                      +  ForceQPrangeDistr                                        &
                                                      +  ForceQPrangeMass
            m%BldInternalForceFE(4:6,idx_FE_all-1  )  =  m%BldInternalForceFE(4:6,idx_FE_all-1  )                 &
                                                      +  cross_product( MomentArmDistr, ForceQPrangeDistr(1:3) )  &
                                                      +  cross_product( MomentArmMass , ForceQPrangeMass(1:3)  )

         endif

            ! Step in through all the FE nodes inboard of the end
         do idx_FE=p%nodes_per_elem-1,1,-1
            ...
            !  Distributed loads
            !     Aerodynamic loads and driver distributed loads
            !     --> NOTE: the distributed loads and mass must terms must be handled separately
            !------------------------------------------------------------------------------------

               ! Step through the list of QP's that contribute (stored in parameter for optimization)
               ! The calculations consider the range from idx_qp to idx_qp+1
            do idx_qp=p%QPrangeOverlapFE(2,idx_FE,nelem),p%QPrangeOverlapFE(1,idx_FE,nelem),-1

                     ! Contributions from each QP for the integration range
                  ContribThisQPdistr = p%Jacobian(idx_qp,nelem)   * ( m%DistrLoad_QP(1:6,idx_qp,nelem) )
                  ContribThisQPmass  = p%Jacobian(idx_qp,nelem)   * ( m%qp%Fg(1:6,idx_qp,nelem) - m%qp%Fi(1:6,idx_qp,nelem) )
                  ContribNextQPdistr = p%Jacobian(idx_qp+1,nelem) * ( m%DistrLoad_QP(1:6,idx_qp+1,nelem) )
                  ContribNextQPmass  = p%Jacobian(idx_qp+1,nelem) * ( m%qp%Fg(1:6,idx_qp+1,nelem) - m%qp%Fi(1:6,idx_qp+1,nelem) )
                  ...
                  ForceQPrangeDistr =  p%QPtWghtIntForceFE(idx_qp  ,2,idx_FE,nelem) * ContribThisQPdistr &   ! outboard this QP
                                    +  p%QPtWghtIntForceFE(idx_qp+1,1,idx_FE,nelem) * ContribNextQPdistr     ! inboard next QP
                  ForceQPrangeMass  =  p%QPtWghtIntForceFE(idx_qp  ,2,idx_FE,nelem) * ContribThisQPmass  &   ! outboard this QP
                                    +  p%QPtWghtIntForceFE(idx_qp+1,1,idx_FE,nelem) * ContribNextQPmass      ! inboard next QP


                     ! Find center of integration range for moment arm: (store in MomentArmDistr)
                  ...
                  MomentArmDistr = MomentArmDistr - (p%uuN0(1:3,idx_FE,  nelem) + x%q(1:3,idx_FE_all))         ! FE node we are putting this on.
                  MomentArmMass  = MomentArmMass  - (p%uuN0(1:3,idx_FE,  nelem) + x%q(1:3,idx_FE_all))         ! FE node we are putting this on.

                     ! Add forces and moments -- at this FE
                  m%BldInternalForceFE(1:6,idx_FE_all)   =  m%BldInternalForceFE(1:6,idx_FE_all)                     &
                                                         +  ForceQPrangeDistr                                        &
                                                         +  ForceQPrangeMass
                  m%BldInternalForceFE(4:6,idx_FE_all)   =  m%BldInternalForceFE(4:6,idx_FE_all)                     &
                                                         +  cross_product( MomentArmDistr, ForceQPrangeDistr(1:3) )  &
                                                         +  cross_product( MomentArmMass , ForceQPrangeMass(1:3)  )
            ...
            m%BldInternalForceFE(1:6,idx_FE_all)   =  m%BldInternalForceFE(1:6,idx_FE_all)   &
                                                   +  m%PointLoadLcl(1:6,idx_FE_all)

               ! Transfer force from next FE node outboard
            m%BldInternalForceFE(1:3,idx_FE_all)   =  m%BldInternalForceFE(1:3,idx_FE_all)   &
                                                   +  m%BldInternalForceFE(1:3,idx_FE_all+1)

               ! Transfer moment from next FE node outboard
            m%BldInternalForceFE(4:6,idx_FE_all)   =  m%BldInternalForceFE(4:6,idx_FE_all)   &
                                                   +  m%BldInternalForceFE(4:6,idx_FE_all+1) &
                                                   +  cross_product( MomentArmPoint, m%BldInternalForceFE(1:3,idx_FE_all+1) )
         ...
         if ( p%QPtN(1) >= p%GLL_Nodes(1) ) then
            ...
            ContribThisQPdistr=  p%Jacobian(idx_qp,nelem) * ( m%DistrLoad_QP(1:6,idx_qp,nelem) )
            ContribThisQPmass =  p%Jacobian(idx_qp,nelem) * ( m%qp%Fg(1:6,idx_qp,nelem) - m%qp%Fi(1:6,idx_qp,nelem) )
            ForceQPrangeDistr =  p%QPtWghtIntForceFE(idx_qp,1,idx_FE,nelem) * ContribThisQPdistr      ! inboard of this QP
            ForceQPrangeMass  =  p%QPtWghtIntForceFE(idx_qp,1,idx_FE,nelem) * ContribThisQPmass       ! inboard of this QP

               ! moment arm for force to moment contribution (midpoint of the integration range for this piece to the previous FE)
            MomentArmDistr = ( (p%uuN0(1:3,idx_FE  ,nelem) + x%q(1:3,idx_FE_all  )) + (p%uu0(1:3,idx_qp,nelem) + m%qp%uuu(1:3,idx_qp,nelem)) ) / 2.0_BDki
            MomentArmMass  = ( (p%uuN0(1:3,idx_FE  ,nelem) + x%q(1:3,idx_FE_all  )) + (p%uu0(1:3,idx_qp,nelem) + m%qp%uuu(1:3,idx_qp,nelem)) ) / 2.0_BDki
            MomentArmMass  = ( (p%uuN0(1:3,idx_FE  ,nelem) + x%q(1:3,idx_FE_all  ))*MassThisFE          &
                                 + (p%uu0(1:3,idx_qp,nelem) + m%qp%uuu(1:3,idx_qp,nelem)*p%qp%mmm(idx_qp,nelem)) ) / &
                                 (MassThisFE + p%qp%mmm(idx_qp,nelem))
...
SUBROUTINE BD_DistrLoadCopy( p, u, m, RampScaling )
   ...
   ! Apply scaling factor to the Applied Forces and store
   DO i=1,p%node_total
...
!FIXME: this doesn't correctly capture the x_cm and y_cm factors!!!!!
       elem_CG(1:3) = elem_CG(1:3) + p%QPtWeight(idx_qp) * p%Jacobian(idx_qp,nelem) * mmm * NQPpos(1:3,idx_qp)
!FIXME: this is not correctly reporting the moment of inertia for a straight blade about the z axis!!!!
...
!FIXME: suspect this term is missing, but need to check tensor math
!                          p%QPtWeight(idx_qp) * p%Jacobian(idx_qp,nelem) * EMass0_GL(4:6,4:6,idx_qp)
