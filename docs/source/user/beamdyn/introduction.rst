...
The module has been coupled into the OpenFAST aero-hydro-servo-elastic wind
...
couples to OpenFAST, and provides new capabilities for modeling
...
When coupled with OpenFAST, loads and responses are transferred between
BeamDyn, ElastoDyn, ServoDyn, and AeroDyn via the OpenFAST driver program
...
for the coupled interactions between BeamDyn and other modules in OpenFAST.
When coupled to OpenFAST, BeamDyn replaces the more simplified blade
...
When uncoupled from OpenFAST, the root motion (boundary condition) and
...
.. figure:: figs/flow_chart.jpg
   ...
   Coupled interaction between BeamDyn and OpenFAST
...
is coupled to OpenFAST, the blade analysis node discretization may be
independent between BeamDyn and AeroDyn.

This document is organized as follows. Section :ref:`running-beamdyn` details how to
obtain the BeamDyn and OpenFAST software archives and run either the
stand-alone version of BeamDyn or BeamDyn coupled to OpenFAST.
